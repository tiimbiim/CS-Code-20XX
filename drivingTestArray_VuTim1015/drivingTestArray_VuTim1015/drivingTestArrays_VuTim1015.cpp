...
const int MAX = 20;		//constant value for array length
...
int main() {
	...
	if (arrLenQ != arrLenA) {
		...
		cout << "\nLength of questions and answers are different.\n";

	}
	else {
		cout << arrLenQ << "--" << arrLenA;
...
int fillResponseArray(const string questions[], char response[], int len) {
	cout << "\nThe test is now beginning.\n";

	for (len = 0; len < MAX; len++) {

		cout << "\n" << questions[len];
		cout << "Please type your answer (in capital letters)." << endl;
		cin >> response[len];
